...
public class Web_Scheduled_Campaigns {
	...
	public Web_Scheduled_Campaigns(WebDriver driver, ExtentTest test) {
		this.driver = driver;
	...
	@FindBy(xpath = "//span[@id='select2-ui_txtCampaignName-container']")
	private WebElement rssCampaignName;
	
	@FindBy(xpath = "//input[@id='ui_txtRssUrl']")
	private WebElement rssUrl;
	
	@FindBy(xpath = "//span[@id='select2-campgroupname-container']")
	private WebElement rssGroup;
	
	@FindBy(xpath = "//select[@id='id_selectHour']")
	private WebElement rssHr;
	
	@FindBy(xpath = "//select[@id='id_selectMinutes']")
	private WebElement rssMinutes;
	
	@FindBy(xpath = "//button[@id='ui_btnNext']")
	private WebElement rssNextButton;
	
	@FindBy(xpath = "//input[@id='ui_MachineId']")
	private WebElement machineID;
	
	@FindBy(xpath = "//input[@id='ui_txtCampaignTestMachineId']")
	private WebElement testCampaignMachineID;
	
	@FindBy(xpath = "//span[@id='select2-campgroupname-container']//following::input[9]")
	private WebElement rssGroupSearch;

	@FindBy(xpath = "//body/span[1]/span[1]/span[1]/input[1]")
	private WebElement campaignNameDropdownSearch;
	
	@FindBy(xpath = "//span[@id='select2-ui_txtCampaignName-container']//following::input[10]")
	private WebElement rssCampaignNameDropdownSearch;
	...
	private WebElement rssCampaignTable;
	...
	public void enter_RSSGroup(String input_RSSGroup) {
		try {
			applyWait.waitForElementToBeClickable(rssGroup, DefineConstants.explicitWait_30).click();
			rssGroupSearch.clear();
			rssGroupSearch.sendKeys(input_RSSGroup);
			rssGroupSearch.sendKeys(Keys.ENTER);
			Screenshots.takeScreenshot(driver, "User entered RSS Group as " + input_RSSGroup);
			test.log(Status.INFO, "User entered RSS Group as " + input_RSSGroup);
			Log.info("User entered RSS Group as " + input_RSSGroup);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	
	public void enter_RSSHr(String input_RSSHr) {
		try {
			applyWait.waitForElementToBeClickable(rssHr, DefineConstants.explicitWait_30);
			dropDown.selectByValue(rssHr, input_RSSHr);
			Screenshots.takeScreenshot(driver, "User entered RSS Hr as " + input_RSSHr);
			test.log(Status.INFO, "User entered RSS Hr as " + input_RSSHr);
			Log.info("User entered RSS Hr as " + input_RSSHr);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	
	public void enter_RSSMinutes(String input_RSSMinutes) {
		try {
			applyWait.waitForElementToBeClickable(rssMinutes, DefineConstants.explicitWait_30);
			dropDown.selectByValue(rssMinutes, input_RSSMinutes);
			Screenshots.takeScreenshot(driver, "User entered RSS Minutes as " + input_RSSMinutes);
			test.log(Status.INFO, "User entered RSS Minutes as " + input_RSSMinutes);
			Log.info("User entered RSS Minutes as " + input_RSSMinutes);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	
	public void click_RSSNextButton() {
		try {
			applyWait.waitForElementToBeClickable(rssNextButton, DefineConstants.explicitWait_30);
			rssNextButton.click();
			Screenshots.takeScreenshot(driver, "User clicked next button after entering RSS minutes");
			test.log(Status.INFO, "User clicked next button after entering RSS minutes");
			Log.info("User clicked next button after entering RSS minutes");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}

	public void enter_MachineID(String input_MachineID) {
		try {
			applyWait.waitForElementToBeClickable(machineID, DefineConstants.explicitWait_30);
			machineID.sendKeys(input_MachineID);
			Screenshots.takeScreenshot(driver, "User entered "+input_MachineID+ " as machine ID");
			test.log(Status.INFO, "User entered "+input_MachineID+ " as machine ID");
			Log.info("User entered "+input_MachineID+ " as machine ID");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
...
//			javascriptClick.sendKeys(campaignNameDropdownSearch,input_CampaignName);
	...
	public void select_RSSCampaignName(String input_RSSCampaignName) {
		try {
			applyWait.waitForElementToBeClickable(rssCampaignName, DefineConstants.explicitWait_30).click();
			applyWait.waitForElementToBeClickable(rssCampaignNameDropdownSearch, DefineConstants.explicitWait_30).sendKeys(input_RSSCampaignName);
//			javascriptClick.sendKeys(campaignNameDropdownSearch,input_CampaignName);
			rssCampaignNameDropdownSearch.sendKeys(Keys.ENTER);
			Screenshots.takeScreenshot(driver, input_RSSCampaignName + " is selected as campaign name");
			test.log(Status.INFO, input_RSSCampaignName + " is selected as campaign name");
			Log.info(input_RSSCampaignName + " is selected as campaign name");
		} catch (Exception e) {
			e.printStackTrace();
		}		
	}
	
	public void enter_RSSUrl(String input_RSSUrl) {
		try {
			applyWait.waitForElementToBeClickable(rssUrl, DefineConstants.explicitWait_30);
			rssUrl.sendKeys(input_RSSUrl);
			Screenshots.takeScreenshot(driver, "User entered RSS URL");
			test.log(Status.INFO, "User entered "+input_RSSUrl+ " as RSS URL");
			Log.info("User entered "+input_RSSUrl+ " as RSS URL");
		} catch (Exception e) {
			e.printStackTrace();
		}		
	}
	...
	public void click_NextButton_GroupDetails() {
		try {
	...
	public void select_TestCampaign(String input_TestCampaign,String value) {
		try {
			...
			if (input_TestCampaign.equals("Individual")) {
				testCampaign.click();
				applyWait.waitForElementToBeClickable(testCampaignMachineID, DefineConstants.explicitWait_30).sendKeys(value);
				Screenshots.takeScreenshot(driver, "User clicked "+input_TestCampaign+" and entered machine ID as " + value);
				test.log(Status.INFO, "User clicked "+input_TestCampaign+" and entered machine ID as " + value);
				Log.info("User clicked "+input_TestCampaign+" and entered machine ID as " + value);
				...
				Screenshots.takeScreenshot(driver, "User clicked "+input_TestCampaign+" and selected group as " + value);
				test.log(Status.INFO, "User clicked "+input_TestCampaign+" and selected group as " + value);
				Log.info("User clicked "+input_TestCampaign+" and selected group as " + value);
			}			
		} catch (Exception e) {
			e.printStackTrace();
		}		
	...
	public void verifyRSSInputValue(String input_SearchCampaigns) {
		applyWait.waitforPresenceOfElementLocated(rssCampaignTable, DefineConstants.explicitWait_30);
		tableRows = rssCampaignTable.findElements(By.tagName("tr"));
		System.out.println("value is " + input_SearchCampaigns);
		for (int row = 0; row < tableRows.size(); row++) {
			tableColums = tableRows.get(row).findElements(By.tagName("td"));
			for (int column = 0; column < tableColums.size(); column++) {
